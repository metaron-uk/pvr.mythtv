...
extern "C" {
  ...
  const unsigned int PVR_TIMER_TYPE_REQUIRES_EPG_SERIES_ON_CREATE     = 0x00800000; /*!< @brief this type should not appear on any create menus unless associated with an EPG tag with 'series' attributes (EPG_TAG.iSeriesNumber > 0 || EPG_TAG.iEpisodeNumber > 0 || EPG_TAG.iEpisodePartNumber > 0). Implies PVR_TIMER_TYPE_REQUIRES_EPG_TAG_ON_CREATE */
  ...
  typedef struct PVR_TIMER {
    ...
    char            strSubtitle[PVR_ADDON_NAME_STRING_LENGTH];
                                                               /*!< @brief (optional) a subtitle for this timer */
  ...
  typedef struct PVR_RECORDING {
    ...
    bool   bExpiresNever;                                 /*!< @brief (optional) the backend will never expire (and auto-delete) this recording */
    bool   bExpiresSoon;                                  /*!< @brief (optional) the backend will expire (and auto-delete) this recording 'soon'. Precise defintition is backend/pvr client specific */
