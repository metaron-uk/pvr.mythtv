...
extern "C" {
  ...
  const unsigned int PVR_TIMER_TYPE_REQUIRES_EPG_SERIES_ON_CREATE     = 0x00800000; /*!< @brief this type should not appear on any create menus unless associated with an EPG tag with 'series' attributes (EPG_TAG.iSeriesNumber > 0 || EPG_TAG.iEpisodeNumber > 0 || EPG_TAG.iEpisodePartNumber > 0) */
  ...
  typedef struct PVR_TIMER {
    ...
    char            strEpisodeName[PVR_ADDON_NAME_STRING_LENGTH];
                                                               /*!< @brief (optional) episode name (also known as subtitle) for this timer */
