...
bool MythScheduleHelper75::FixRule(MythRecordingRule& rule) const
{
  bool ruleIsOK = true;
  if (rule.StartTime() == rule.EndTime())
  {
    rule.SetEndTime(rule.EndTime() + 2);
    XBMC->Log(LOG_INFO, "75::%s: Rule %s (%u) has zero duration which the backend would reject. End time increased by 2 seconds.",
  ...
  switch (rule.SearchType())
  {
    case Myth::ST_PowerSearch:
      ...
      {
        XBMC->Log(LOG_INFO, "75::%s: Rule %s (%u) has search type PowerSearch with no Subtitle. It will probably do nothing.",
                  __FUNCTION__, rule.Title().c_str(), rule.RecordID());
        ruleIsOK = false;
      }
    case Myth::ST_TitleSearch:
    ...
    case Myth::ST_PeopleSearch:
      ...
      {
        XBMC->Log(LOG_INFO, "75::%s: Rule %s (%u) has search type %u and no Description. It may severly degrade backend performance.",
                  __FUNCTION__, rule.Title().c_str(), rule.RecordID(), rule.SearchType());
        ruleIsOK = false;
      }
      else if (rule.Description().length() <= 3)
      {
        XBMC->Log(LOG_INFO, "75::%s: Rule %s (%u) has search type %u and a very short Description. It may degrade backend performance.",
                  __FUNCTION__, rule.Title().c_str(), rule.RecordID(), rule.SearchType());
        ruleIsOK = false;
      }
      break;
  ...
  {
    XBMC->Log(LOG_INFO, "75::%s: Rule %s (%u) has no associated channel, which the backend will reject.",
              __FUNCTION__, rule.Title().c_str(), rule.RecordID());
    ruleIsOK = false;
  }
  if (!ruleIsOK)
