...
PVR_ERROR PVRClientMythTV::GetTimers(ADDON_HANDLE handle)
{
  ...
  for (MythTimerEntryList::const_iterator it = entries.begin(); it != entries.end(); ++it)
  {
    ...
    tag.bFullTextEpgSearch = (*it)->isFullTextEpgSearch;
    ...
    case TIMER_TYPE_RULE_INACTIVE:       // Rule Disabled
    case TIMER_TYPE_UPCOMING_ALTERNATE:  // Alternateve time
    case TIMER_TYPE_UPCOMING_RECORDED:   // Already Currently Recorded
    case TIMER_TYPE_UPCOMING_EXPIRED:    // Previously Recorded and Expired
...
MythTimerEntry PVRClientMythTV::PVRtoTimerEntry(const PVR_TIMER& timer, bool checkEPG)
{
  ...
  entry.isFullTextEpgSearch = timer.bFullTextEpgSearch;
